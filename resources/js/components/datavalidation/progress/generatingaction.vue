                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title">
                                {{ translate("generatingaction.header") }}
                            </h3>
                            <div class="card-tools">
                                ...
                                >
                                    ...
                                    {{ translate("generatingaction.addnew") }}
                                </button>
                            </div>
                        </div>
                        <!-- /.card-header -->
                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" v-show="!editmode">
                                {{ translate("generatingaction.create.header") }}
                            </h5>
                            <h5 class="modal-title" v-show="editmode">
                                {{ translate("generatingaction.update.header") }}
                            </h5>
                            <button
                                type="button"
                                class="close"
                        ...
                        >
                            ...
                            <div class="modal-footer">
                                <button
                                    ...
                                    data-dismiss="modal"
                                >
                                    {{ translate("generatingaction.actions.close") }}
                                </button>
                                <button
                                    v-show="editmode"
                                    type="submit"
                                ...
                                >
                                    {{ translate("generatingaction.actions.update") }}
                                </button>
                                <button
                                    v-show="!editmode"
                                    type="submit"
                                ...
                                >
                                    {{ translate("generatingaction.actions.create") }}
                                </button>
                            </div>
                        </form>
                    </div>
...
export default {
    ...
    methods: {
        ...
        updateItem() {
            ...
            this.form
                .put("/generatingaction/" + this.form.id)
                .then(response => {
                    // success
                    $("#addNew").modal("hide");
                    Toast.fire({
        ...
        deleteItem(item) {
            Swal.fire({
                title: window.translate("generatingaction.alert.delete_building_title"),
                text:
                    window.translate("generatingaction.alert.delete_building_text") +
                    ` [${item.status}]`,
                showCancelButton: true,
                confirmButtonColor: "#d33",
                cancelButtonColor: "#3085d6",
                cancelButtonText: window.translate(
                    "generatingaction.alert.delete_building_cancel_button_text"
                ),
                confirmButtonText: window.translate(
                    "generatingaction.alert.delete_building_confirm_button_text"
                )
            }).then(result => {
                // Send request to the server
                if (result.value) {
                    this.form
                        .delete("/generatingaction/" + item.id)
                        .then(() => {
                            Swal.fire(
                                window.translate(
                                    "generatingaction.alert.comfirm_delete_title"
                                ),
                                window.translate(
                                    "generatingaction.alert.confirm_delete_message"
                                ),
                                "success"
                            );
                            // Fire.$emit('AfterCreate');
