                    ...
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" v-show="!editmode">
                                {{ translate("areatxrx.create.header") }}
                            </h5>
                            <h5 class="modal-title" v-show="editmode">
                                {{ translate("areatxrx.update.header") }}
                            </h5>
                            <button
                                type="button"
                                class="close"
                        ...
                        >
                                ...
                                <div class="row">
                                    <div class="col-sm-12">
                                        <div class="form-group">
                                            <label>Area Txrx</label>
                                            <input
                                                v-model="form.AreaTxrx"
                                                type="text"
                                                class="form-control"
                                                placeholder="Enter your name area txrx..."
                                                :class="{
                            ...
                            <div class="modal-footer">
                                <button
                                    ...
                                    data-dismiss="modal"
                                >
                                    {{ translate("areatxrx.actions.close") }}
                                </button>
                                <button
                                    v-show="editmode"
                                    type="submit"
                                ...
                                >
                                    {{ translate("areatxrx.actions.update") }}
                                </button>
                                <button
                                    v-show="!editmode"
                                    type="submit"
                                ...
                                >
                                    {{ translate("areatxrx.actions.create") }}
                                </button>
                            </div>
                        </form>
                    </div>
...
export default {
    ...
    methods: {
        ...
        deleteItem(item) {
            Swal.fire({
                title: window.translate("areatxrx.alert.delete_building_title"),
                text:
                    window.translate("areatxrx.alert.delete_building_text") +
                    ` [${item.areaTxrx}]`,
                showCancelButton: true,
                confirmButtonColor: "#d33",
                cancelButtonColor: "#3085d6",
                cancelButtonText: window.translate(
                    "areatxrx.alert.delete_building_cancel_button_text"
                ),
                confirmButtonText: window.translate(
                    "areatxrx.alert.delete_building_confirm_button_text"
                )
            }).then(result => {
                // Send request to the server
                if (result.value) {
                    this.form
                        .delete("/areatxrx/" + item.id)
                        .then(() => {
                            Swal.fire(
                                window.translate(
                                    "areatxrx.alert.comfirm_delete_title"
                                ),
                                window.translate(
                                    "areatxrx.alert.confirm_delete_message"
                                ),
                                "success"
                            );
                            // Fire.$emit('AfterCreate');
