...
export default {
    ...
    methods: {
        ...
        deleteItem(item) {
            Swal.fire({
                title: window.translate("status.alert.delete_building_title"),
                text:
                    window.translate("status.alert.delete_building_text") +
                    ` [${item.status}]`,
                showCancelButton: true,
                confirmButtonColor: "#d33",
                cancelButtonColor: "#3085d6",
                cancelButtonText: window.translate(
                    "status.alert.delete_building_cancel_button_text"
                ),
                confirmButtonText: window.translate(
                    "status.alert.delete_building_confirm_button_text"
                )
            }).then(result => {
                ...
                if (result.value) {
                    this.form
                        ...
                        .then(() => {
                            Swal.fire(
                                window.translate(
                                    "status.alert.comfirm_delete_title"
                                ),
                                window.translate(
                                    "status.alert.confirm_delete_message"
                                ),
                                "success"
                            );
                            // Fire.$emit('AfterCreate');
