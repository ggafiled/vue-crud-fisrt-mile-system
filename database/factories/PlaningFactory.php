...
class PlaningFactory extends Factory
{
    ...
    public function definition()
    {
        return [
            ...
            'confirming' => $this->faker->dateTime($max = 'now', $timezone = 'utc'),
            'team_id' => $this->faker->numberBetween($min = 1, $max = 20),
            'remark' => $this->faker->sentence($nbWords = 6, $variableNbWords = true) ,
            'date' => $this->faker->dateTime($max = 'now', $timezone = 'utc'),
            'time' => $this->faker->dateTime($max = 'now', $timezone = 'utc'),
            'status' => NULL,
            'subStatus' => NULL,
            'dateConnect' => $this->faker->dateTime($max = 'now', $timezone = 'utc'),
            'dateDisconnect' => $this->faker->dateTime($max = 'now', $timezone = 'utc'),
        ];
    }
}
